...
export class MovieValidation {
    ...
    .object({
      ...
      price: z.number({
        error: (val) =>
          val.input === undefined
            ? "price harus diisi"
            : "price harus berupa number",
      }),
      available: z.boolean({
        error: (val) =>
          val.input === undefined
            ? "available harus diisi"
            : "available harus berupa boolean",
      }),
      genreId: z.number({
        error: (val) =>
          val.input === undefined
            ? "genreId harus diisi"
            : "genreId harus berupa number",
      }),
      bonus: z
        .string()
        .transform((val) => {
      ...
      seats: z
        .number({
          error: (val) =>
            val.input === undefined
              ? "seats harus diisi"
              : "seats harus berupa number",
        })
        .refine((val) => val <= 30, "seats tidak boleh lebih dari 30"),
    })
    .strict() satisfies ZodType<
    Omit<MovieCreateType, "thumbnail" | "theaterId"> & { theaterId: string }
  >;
  ...
  static readonly UPDATE = z
    .object({
      title: z
        ...
        .min(3, "title minimal 1 karakter")
        .optional(),
      description: z
        .string({
          error: (val) =>
        ...
        .min(3, "description minimal 1 karakter")
        .optional(),
      price: z
        .number({
          error: (val) =>
  ...
  static readonly SEATS_BOOKED = z
    .object({
      ...
      times: z.string("Times Harus di isi"),
    })

    .strict() satisfies ZodType<Omit<BookedCreateType, "movieId">>;
}
