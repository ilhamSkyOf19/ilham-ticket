...
const DashboardLayout: FC = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (even: MouseEvent) => {
      ...
      if (
        sidebarRefCurrent &&
        !sidebarRefCurrent.contains(even.target as Node)
      ) {
  ...
  return (
    <div className="w-full flex flex-row justify-start items-start gap-2 relative">
      ...
      <div className="fixed top-0 left-0 w-full h-13 bg-white/10 backdrop-blur-sm flex justify-between items-center md:hidden px-2 z-50">
        {/* bars & back*/}
        {pathDetail ? (
          <button
            type="button"
      ...
      <SideBar
        ref={useRefSidebar as RefObject<HTMLDivElement>}
