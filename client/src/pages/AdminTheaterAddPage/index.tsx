...
const AdminTheaterAddPage: FC = () => {
  ...
  const {
    ...
    setValue,
    clearErrors,
  } = useForm<TheaterCreateType | TheaterUpdateType>({
    // default values
    defaultValues: {
    ...
    resolver: zodResolver(
      theater ? TheaterValidation.UPDATE : TheaterValidation.CREATE
    ),
  });
  ...
  const { isPending, mutateAsync } = useMutation({
    mutationFn: async (data: TheaterCreateType | TheaterUpdateType) => {
      theater
        ? await TheaterService.update(
            theater.data?.id || 0,
            data as TheaterUpdateType
          )
        : await TheaterService.create(data as TheaterCreateType);
    },
    onError: (error) => {
  ...
  return (
    <div className="w-full flex flex-col justify-start items-start py-18 px-2">
      {/* header */}
      <HeaderDashboardData
        title={theater ? "Update Data Theater" : "Add Data Theater"}
      />
