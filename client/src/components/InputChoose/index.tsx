...
const InputChoose: FC<Props> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        refModalChoose.current &&
        !refModalChoose.current.contains(target) &&
        !refButtonModalChoose.current?.contains(targetButton)
      ) {
  ...
  return (
    <div className="w-full flex flex-col justify-start items-start relative z-20">
      {/* label */}
      <label htmlFor={name} className="text-base font-medium text-white mb-2">
        {label}
      </label>
      ...
      >
        ...
        <button
          type="button"
          className="absolute bg-transparent w-full h-full border-none outline-none z-10"
          onClick={handleModalChoose}
          ref={refButtonModalChoose}
        />
          ...
          {fieldChoose.length > 0 ? (
            fieldChoose.map((item: { id: number; name: string }, index) => (
              <button
                ...
                className="w-full py-3 hover:bg-gray-400 hover:text-white text-left px-5 text-black font-semibold capitalize"
              >
                {item.name}
              </button>
            ))
