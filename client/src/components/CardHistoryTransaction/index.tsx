...
const CardHistoryTransaction: FC<Props> = ({
  plus,
  nominal,
  name,
  thumbnail,
  ...
  return (
    <div className="w-full flex flex-row justify-between items-center overflow-hidden">
      ...
      <div className="flex-2 flex flex-row justify-start items-center gap-3">
        ...
        <div
          className={clsx(
            "h-22 w-22 rounded-2xl overflow-hidden flex flex-col justify-center items-center",
            plus && "bg-white/10"
          )}
        >
          <img
            src={thumbnail ?? cards}
            alt="thumbnail"
            className={clsx(!plus ? "object-cover w-full h-full" : "w-9 h-9")}
            loading="lazy"
          />
        </div>
...
const Nominal: FC<NominalProps> = ({ plus, nominal }) => {
  return (
    <div className="flex flex-row justify-start items-start">
      {/* plus */}
      <p className={clsx("text-xs", plus ? "text-green-500" : "text-red-500")}>
        <span className="mr-0.5">{plus ? "+" : "-"}</span>
