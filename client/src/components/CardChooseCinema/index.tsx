...
const CardChooseCinema: FC<Props> = ({
  handleClickTheater,
  theater,
  active,
  error,
  ...
  return (
    <button
      ...
      key={theater.id}
      className={clsx(
        "w-40 h-28 bg-white/10 rounded-2xl flex flex-col justify-start items-start px-3 py-3 gap-3 transition-all duration-200 ease-in-out hover:bg-white/20",
        active
    ...
    >
      ...
      {/* location */}
      <ComponentInfo icon={iconLocation} label={theater.city} />
    </button>
  );
};
